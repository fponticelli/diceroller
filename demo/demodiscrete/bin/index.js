...
Demo.__name__ = true;
Demo.main = function() {
	var d6 = probability_Discrete.die(6);
	console.log("d6");
	...
	var d8 = probability_Discrete.die(8);
	var d12 = probability_Discrete.die(12);
	var dexpr;
	console.log("2d8 / d12 (for some reason)");
	dexpr = d8.binary(d8,function(x1,y1) {
		return x1 + y1;
	}).binary(d12,function(x2,y2) {
		return x2 / y2;
	});
	console.log(dexpr.weights());
	console.log(dexpr.probabilities());
	console.log(dexpr.values());
};
Math.__name__ = true;
var Std = function() { };
Std.__name__ = true;
...
var probability_Discrete = function(weights,values) {
	...
	while(_g2 < _g1) {
		var i = _g2++;
		var this1 = { _0 : 0, _1 : 0.0};
		_g.push(this1);
	}
	this.weighted_values = _g;
	var _g21 = 0;
	var _g11 = n;
	while(_g21 < _g11) {
		var i1 = _g21++;
		var this2 = { _0 : weights[i1], _1 : values[i1]};
		this.weighted_values[i1] = this2;
	}
	this.compact();
};
probability_Discrete.__name__ = true;
probability_Discrete.die = function(n) {
	var _g = [];
	var _g2 = 0;
	var _g1 = n;
	while(_g2 < _g1) {
	...
	return new probability_Discrete(tmp,_g11);
};
probability_Discrete.compare = function(x,y) {
	if(x._1 == y._1) {
		return 0;
	} else if(x._1 < y._1) {
		return -1;
...
probability_Discrete.prototype = {
	length: function() {
	...
	,compact: function() {
		this.weighted_values.sort(probability_Discrete.compare);
		var old_weights = this.weights();
		...
		var j = 0;
		var _g1 = 1;
		var _g = old_weights.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(this.weighted_values[j]._1 == old_values[i]) {
				var this2 = { _0 : this.weighted_values[j]._0 + old_weights[i], _1 : this.weighted_values[j]._1};
				this.weighted_values[j] = this2;
			} else {
				++j;
				var this3 = { _0 : old_weights[i], _1 : old_values[i]};
				this.weighted_values[j] = this3;
			}
		}
	}
	,unary: function(f) {
		return new probability_Discrete(this.weights(),this.values().map(f));
	}
	,binary: function(other,f) {
		...
		return new probability_Discrete(weights,values);
	}
};
